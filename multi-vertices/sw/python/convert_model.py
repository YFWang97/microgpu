...
###############################################################################
# 4) Extracting Data from a Single Primitive
###############################################################################
def extract_primitive_data(gltf, primitive, gltf_file_path, convert_quads=False):
    ...
    # Convert quads if requested (naively, no CCW check)
...
###############################################################################
# 6) Main Script (Combine, Then Export to Files)
###############################################################################
def main():
    if len(sys.argv) < 2:
        print("Usage: python export_noccw.py path/to/model.gltf [--convert-quads]")
        sys.exit(1)
    ...
    # We combine all positions into 'all_vertices', all indices into 'all_indices',
    # offsetting indices so they reference the correct new positions.
    all_vertices = []
    all_indices = []
    current_offset = 0
    ...
    with open("vertices.txt", "w", encoding="utf-8") as vfile:
        for v in all_vertices_np:
            vfile.write(f"{v[0]} {v[1]} {v[2]}\n")

    # Write "indices.txt" (3 indices per line)
    with open("indices.txt", "w", encoding="utf-8") as ifile:
        for i in range(0, len(all_indices_np), 3):
            i0 = all_indices_np[i]
            i1 = all_indices_np[i + 1]
            i2 = all_indices_np[i + 2]
            ifile.write(f"{i0} {i1} {i2}\n")

    print("Export complete.")
    print(f"Wrote {len(all_vertices_np)} vertices to 'vertices.txt'.")
    print(f"Wrote {len(all_indices_np)//3} triangles to 'indices.txt' (no CCW check).")
